...
#Se relaliza el primer recorrido para un unico hilo y se mide su tiempo de ejecucion
    hilo = threading.Thread(target=potencia,args=(i, lista, elemento,'t1.solo'))
    start=time.time()
    hilo.start()
    hilo.join()
...
#Se realiza el recorrido para los 4 hilos de manera que se reparten el trabajo equitativo y se mide el tiempo de ejecucion de este proceso
    rango = elemento // 4;
    hilo1 = threading.Thread(target=potencia, args=[i, lista, i + rango,'t1.multiple'])
    hilo2 = threading.Thread(target=potencia, args=[i + 1 + rango, lista, i + 2 * rango,'t2.multiple'])
    hilo3 = threading.Thread(target=potencia, args=[i + 1 + 2 * rango, lista, i + 3 *rango,'t3.multiple'])
    hilo4 = threading.Thread(target=potencia, args=[i + 1 + 3 * rango, lista, elemento,'t4.multiple'])
    start4 = time.time() 
    hilo1.start()  
    hilo2.start()
    hilo3.start()
